...
const useStaticData = () => {
  return useMemo(() => ({
    ...
    textVariants: (shouldReduce) => ({
      hidden: { opacity: 0, y: shouldReduce ? 0 : 100 },
      visible: { opacity: 1, y: 0, transition: { duration: shouldReduce ? 0 : 1 } },
    }),
    statVariants: (shouldReduce) => ({
      hidden: { opacity: 0, y: shouldReduce ? 0 : 100 },
      visible: (i) => ({
        opacity: 1,
        y: 0,
...
const BoxItem = React.memo(({ text, widths, heights }) => {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true });
  const [isHovered, setIsHovered] = useState(false);
  const shouldReduceMotion = useReducedMotion();
  const hoverHandler = useMemo(
    () => throttle(setIsHovered, 100, { leading: true, trailing: false }),
    []
  );
  ...
  return (
    <motion.div
      ...
      onHoverStart={() => !shouldReduceMotion && hoverHandler(true)}
      onHoverEnd={() => !shouldReduceMotion && hoverHandler(false)}
    >
      <div
      ...
      >
        ...
        <button
          className="absolute flex items-center justify-center text-base sm:text-base md:text-lg lg:text-lg"
          style={{
            transform: shouldReduceMotion ? "translateY(0)" : (isHovered ? "translateY(0)" : "translateY(140%)"),
            transition: shouldReduceMotion ? 'none' : 'transform 500ms'
          }}
        >
          {text}
        </button>
...
const ContactUs = () => {
  const shouldReduceMotion = useReducedMotion();
  const { images, textVariants, statVariants } = useStaticData();
  ...
  const isSectionInView = useInView(sectionRef, observerOptions);
  const is500InView = useInView(stats500Ref, observerOptions);
  const is98InView = useInView(stats98Ref, observerOptions);
  const is85KInView = useInView(stats85KRef, observerOptions);
  const is115InView = useInView(stats115Ref, observerOptions);

  const count500 = useCountUp(is500InView ? 500 : 0, shouldReduceMotion ? 0 : 2000);
  const count98 = useCountUp(is98InView ? 98 : 0, shouldReduceMotion ? 0 : 2000);
  const count85K = useCountUp(is85KInView ? 85000 : 0, shouldReduceMotion ? 0 : 2000);
  const count115 = useCountUp(is115InView ? 115 : 0, shouldReduceMotion ? 0 : 2000);
    ...
    <>
      <div className="flex flex-col items-center py-8 px-4 lg:px-8">
        ...
        <div className="relative w-full max-w-[76rem] mb-16">
          <img
            src="https://cdn.prod.website-files.com/66680ca683883f060b423408/6675aa6a58898eb868eb6294_Banner-11.webp"
            alt="Interior"
              ...
              >
                ...
                >
                  <motion.img
                    ...
                    transition={{ duration: shouldReduceMotion ? 0 : 0.7, delay: index * 0.1 }}
                    viewport={{ once: true, margin: "100px" }}
                    loading="lazy"
                  />
